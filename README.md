...
| 播放历史同步 | PotPlayer 关闭后自动反查最后播放剧集，实时同步播放记录 |
...
│   └── potplayer_monitor.py            # PotPlayer 播放历史同步监控器
├── static/                             # 前端静态资源
...
| POST | `/api/play` | 外部播放器播放（路径安全校验，含 PotPlayer 监控） |
...
### `potplayer_monitor.py` (182 行)
PotPlayer 播放历史同步监控器。当用户从 MovieWall 播放一集后，PotPlayer 可能连续播放下几集，但 MovieWall 无法知道用户实际看到了哪一集。此模块通过在**后台线程轮询 PotPlayer 窗口标题** 来解决这个问题。

**工作原理：**
1. 播放时启动后台线程（`daemon=True`），传入 PotPlayer 进程 PID 和 season_id
2. 每 5 秒调用 `EnumWindows` + `GetWindowText` 读取 PotPlayer 窗口标题
3. 窗口标题格式始终为 `{当前播放文件名} - PotPlayer`，极其稳定
4. 检测到文件名变化后，通过 `episodes` 表按 `season_id + filename` 匹配剧集
5. 匹配成功则写入 `save_history()` 更新播放历史
6. 当 PotPlayer 窗口消失超过 30 秒（用户最小化到托盘后未恢复）或进程退出时停止轮询

**关键技术：**
- 使用 `ctypes` 调用 Win32 API（`EnumWindows`、`GetWindowTextW`），零额外依赖
- 不依赖 PotPlayer 注册表或 IPC，不修改播放器本身
- 不使用不稳定的窗口标题文本识别以外的任何 hack

### `static/app.js` (1124 行)
...
### Q: PotPlayer 连播多集后播放历史没有同步？

MovieWall 通过**轮询 PotPlayer 窗口标题**来检测当前播放的文件。当你在 PotPlayer 中跳转到下一集时，窗口标题会变化，后台监控线程在 5 秒内会自动检测到并更新播放历史。

如果同步没有生效，检查：
1. **PotPlayer 窗口标题格式** — 必须包含 ` - PotPlayer` 后缀。如果使用了非官方修改版或皮肤改变了标题格式，可能无法识别。
2. **进程权限** — MovieWall 需要有权限枚举窗口。以管理员身份运行时通常没有问题。
3. **文件在库中存在** — 监控器会在数据库中按 `season_id + filename` 匹配。如果播放的文件不在 MovieWall 的扫描范围内（例如手动从资源管理器打开的文件），则无法匹配。
4. **日志检查** — 查看 `moviewall.log` 中 `POTPLAYER MONITOR` 和 `POTPLAYER SYNC` 相关的日志条目。

> 注意：MovieWall 只监控**从 MovieWall 内部发起播放**的 PotPlayer 进程。直接双击文件打开 PotPlayer 不会被监控。
